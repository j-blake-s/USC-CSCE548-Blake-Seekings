...
public class DatabaseHarness {
    ...
    System.out.println("=== Starting Full E-Commerce Data Layer Test ===");
            try {
                ...
                // Delete Shipment
                dp.shipment.delete(dp.shipment.readAll().getLast().getShipmentId());
                System.out.println("[PASS] Shipment Deleted");

                // Delete Payment
                dp.payment.delete(dp.payment.readAll().getLast().getPaymentId());
                System.out.println("[PASS] Payment Deleted");

                // Delete Order Item
                dp.orderItem.delete(dp.orderItem.readAll().getLast().getOrderItemId());
                System.out.println("[PASS] Order Item Deleted");

                // Delete Order
                dp.order.delete(dp.order.readAll().getLast().getOrderId());
                System.out.println("[PASS] Order Deleted");

                // Delete Product
                dp.product.delete(dp.product.readAll().getLast().getProductId());
                System.out.println("[PASS] Product Deleted");

                // Delete Customer
                dp.customer.delete(dp.customer.readAll().getLast().getCustomerId());
                System.out.println("[PASS] Customer Deleted");

                // Delete Category
                dp.category.delete(dp.category.readAll().getLast().getCategoryId());
